 ...
 * Dark top nav (52px) + auth guard + mobile bottom tab bar
 */
"use client";
import { useEffect, useState } from "react";
import { useRouter, usePathname } from "next/navigation";
import Link from "next/link";
import { IconLayoutDashboard, IconTarget, IconBriefcase, IconSearch, IconMessage, IconUserCircle, IconChevronDown, IconLogout } from "@tabler/icons-react";
    ...
    <div style={{ minHeight: '100vh', background: 'var(--color-bg)', display: 'flex', flexDirection: 'column' }}>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, background: '#1C1C28', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px' }}>
          ...
          {NAV_LINKS.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link key={href} href={href} style={{
                ...
                fontSize: 12, textDecoration: 'none', height: '100%',
        ...
        <div style={{ position: 'relative' }}>
          <button onClick={() => setMenuOpen((o) => !o)} style={{
            display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#ccc',
            background: 'rgba(255,255,255,0.08)', border: 'none', padding: '5px 12px',
      ...
      <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: '#1C1C28', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-around', borderTop: '0.5px solid var(--color-border)', zIndex: 50 }}>
        {NAV_LINKS.map(({ href, label, icon: Icon }) => {
          const active = pathname.startsWith(href);
          return (
            <Link key={href} href={href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, fontSize: 10, textDecoration: 'none', padding: '8px 12px', color: active ? '#0891B2' : '#666' }}>
              <Icon size={20} />{label}
            </Link>
          );
        })}
