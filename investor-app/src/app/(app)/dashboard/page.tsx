/**
 * Dashboard — hero card, metrics, goals strip, sector allocation, holdings preview
 */
"use client";
import { useEffect, useState, useCallback } from "react";
import Link from "next/link";
import { IconTrendingUp, IconTrendingDown, IconCoins, IconRefresh, IconStack2, IconTarget, IconBuilding, IconSchool, IconShield, IconHome, IconChartPie, IconBriefcase, IconUser, IconArrowRight, IconCalendar } from "@tabler/icons-react";
...
import type { Client, PortfolioSummary, Goal, SectorAllocation, Holding } from "@/types";
...
export default function DashboardPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [cRes, sRes, gRes, secRes, hRes] = await Promise.all([
        api.get('/clients/me'), api.get('/portfolio/summary'),
        api.get('/goals'), api.get('/portfolio/sector-allocation'), api.get('/portfolio/holdings'),
      ]);
      setClient(cRes.data); setSummary(sRes.data); setGoals(gRes.data);
      setSectors(Object.entries(secRes.data as Record<string, number>)
        .map(([sector, weight]) => ({ sector, weight: Number((weight * 100).toFixed(1)) }))
        .sort((a, b) => b.weight - a.weight).slice(0, 6));
      setHoldings((hRes.data as Holding[]).slice(0, 4));
    } catch { setError('Could not load your portfolio. Make sure the API is running.'); }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <HeroCard>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
          <div>
            ...
            <p style={{ fontSize: 28, fontWeight: 500, letterSpacing: '-0.5px', lineHeight: 1, marginBottom: 8 }}>{formatINR(summary?.total_current ?? 0)}</p>
          ...
          <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8 }}>
            <span className="pill" style={{ background: 'rgba(255,255,255,0.2)', color: 'white' }}>{client?.risk_profile ?? 'Not profiled'}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, opacity: 0.85 }}>
              <IconRefresh size={13} /> ₹{((summary?.total_invested ?? 0) * 0.033).toLocaleString('en-IN', { maximumFractionDigits: 0 })} / month SIP
            </div>
          </div>
        </div>
      </HeroCard>
          ...
          : goals.map((g) => (
            <div key={g.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: '0.5px solid var(--color-border)' }}>
              ...
              <div style={{ textAlign: 'right' }}>
                ...
                {g.feasibility && <Pill variant={feasibilityVariant(g.feasibility)} className="mt-1" style={{ fontSize: 10 }}>{g.feasibility}</Pill>}
              </div>
            </div>
          ))}
      </Card>
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={sectors} layout="vertical" margin={{ left: 0, right: 24, top: 0, bottom: 0 }}>
              ...
              <Bar dataKey="weight" radius={[0, 4, 4, 0]}>
                {sectors.map((_, i) => <Cell key={i} fill={i === 0 ? '#0891B2' : `rgba(8,145,178,${0.7 - i * 0.1})`} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </Card>
      ...
      <Card style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 11, color: 'var(--color-text-muted)' }}>Your advisor</p>
          <p style={{ fontSize: 13, fontWeight: 500 }}>{client?.advisor_id ? `Advisor #${client.advisor_id}` : 'Not assigned yet'}</p>
        </div>
