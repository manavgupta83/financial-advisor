 ...
 * Fixed: phone sent as "0000000000" — API requires min 10 chars, UI stays email-only
 */
"use client";
...
function extractError(err: unknown, fallback: string): string {
  ...
  if (Array.isArray(detail) && detail.length > 0) {
    ...
    if (first?.msg) {
      ...
      return String(field) + ": " + String(first.msg);
    }
  }
  return fallback;
}
...
export default function LoginPage() {
  ...
  async function handleRequestOtp(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const { data } = await api.post("/auth/request-otp", { email, phone: "0000000000" });
      if (data.otp_dev) setDevOtp(data.otp_dev);
      setStep("otp");
    } catch (err: unknown) {
      setError(extractError(err, "Could not send OTP. Check the email and try again."));
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleVerifyOtp(e: React.FormEvent) {
    ...
    try {
      ...
      setError(extractError(err, "Invalid or expired OTP. Try again."));
    } finally {
      setLoading(false);
    }
  }
      ...
      <div style={{ display: "flex", background: "#F3F4F6", borderRadius: 20, border: "0.5px solid var(--color-border)", padding: 4, marginBottom: "1.5rem" }}>
        {(["investor", "advisor"] as Role[]).map((r) => (
          <button key={r} onClick={() => { setRole(r); setError(null); }}
            style={{ flex: 1, padding: "8px 0", borderRadius: 20, border: "none", fontSize: 11, fontWeight: 500, cursor: "pointer", textTransform: "capitalize", transition: "all 0.15s", background: role === r ? "var(--color-surface)" : "transparent", color: role === r ? "var(--color-text-primary)" : "var(--color-text-muted)", boxShadow: role === r ? "0 1px 3px rgba(0,0,0,0.06)" : "none" }}>
          ...
          <>
            ...
            <form onSubmit={handleRequestOtp} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                ...
                <input type="email" required autoFocus value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="input" />
              </div>
          ...
          <>
            ...
            {devOtp && (
              <div style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--color-accent-light)", borderRadius: 8, padding: "8px 12px", marginBottom: 16 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 500, color: "var(--color-accent)", cursor: "pointer", textDecoration: "underline" }} onClick={() => setOtp(devOtp)}>{devOtp} (click to fill)</span>
              </div>
            )}
            <form onSubmit={handleVerifyOtp} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                <label style={{ fontSize: 11, color: "var(--color-text-muted)", display: "block", marginBottom: 4 }}>One-time password</label>
                <input type="text" inputMode="numeric" maxLength={6} pattern="\d{6}" required autoFocus value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))} placeholder="123456" className="input" style={{ letterSpacing: "0.2em", fontSize: 18, fontWeight: 500 }} />
              </div>
              {error && <p style={{ fontSize: 11, color: "var(--color-negative)", background: "var(--color-negative-light)", borderRadius: 8, padding: "8px 12px" }}>{error}</p>}
              <button type="submit" disabled={loading} className="btn-primary" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
