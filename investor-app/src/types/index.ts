 ...
 * types/index.ts
 * --------------
 * Shared TypeScript types mirroring the actual FastAPI Pydantic schemas.
 * Verified against api/schemas/*.py — last checked June 2026.
 */
...
export interface Client {
  ...
  onboarding_status?: string;   // not in ClientResponse schema — optional
  advisor_id?: number | null;   // not in ClientResponse schema — optional
}
...
export interface PortfolioSummary {
  ...
  blended_xirr?: number | null;
  blended_cagr?: number | null;
  blended_sharpe?: number | null;  // field is blended_sharpe not sharpe_ratio
}

export interface SectorAllocationItem {
  sector: string;
  weight_pct: number;   // already a percentage e.g. 28.5, NOT a decimal
}

export interface SectorAllocationResponse {
  client_id: number;
  allocations: SectorAllocationItem[];
...
export interface StockExposureResult {
  client_id: number;
  stocks: StockExposureItem[];
  redundant_flags: string[];
  data_gaps: string[];
}
