...
Sections:
  1. Holdings table — fund name, units, avg NAV, invested, current value, gain %
  2. Performance metrics — XIRR, CAGR, Sharpe ratio (via performance_engine)
  3. Sector allocation — bar + pie chart (via sector_engine)
  4. Overlap heatmap — pairwise fund overlap (via overlap_engine)
  5. Add holding form (manual entry for now; NAV auto-fetch deferred to live flow)
...
from engine.performance_engine import (
    compute_cagr, compute_sharpe_ratio,
    simulate_sip_cashflows, compute_xirr,
)
...
# Compute summary directly from enriched holdings (schema has no current_value column)
...
summary = {
    ...
    "absolute_gain":   gain_total,
    "gain_percentage": gain_pct_total,
}

s1, s2, s3, s4, s5 = st.columns(5)
invested = summary.get("total_invested", 0)
current  = summary.get("total_current", 0)
gain     = summary.get("absolute_gain", 0)
gain_pct = summary.get("gain_percentage", 0)
n_hold   = summary.get("num_holdings", 0)

for col, label, val, cls in [
    (s1, "Holdings",       str(n_hold),              ""),
    (s2, "Total Invested", f"₹{invested/1e5:.2f}L",  ""),
    (s3, "Current Value",  f"₹{current/1e5:.2f}L",   ""),
    (s4, "Absolute Gain",  f"₹{gain/1e5:.2f}L",      "positive" if gain>=0 else "negative"),
    (s5, "Gain %",         f"{gain_pct:.2f}%",        "positive" if gain_pct>=0 else "negative"),
]:
    with col:
        st.markdown(f"""
...
if not holdings:
    ...
    display_df = df[[
        "scheme_name", "units", "avg_nav",
        "invested_amount", "current_value", "gain", "gain_pct"
    ]].copy()
    ...
    display_df["Gain (₹)"]     = display_df["Gain (₹)"].apply(lambda x: f"₹{x:,.0f}")
...
if holdings:
    ...
    # Aggregate across holdings for portfolio-level stats
    total_invested = sum(h["invested_amount"] for h in holdings)
    total_current  = sum(h["current_value"]   for h in holdings)

    # Estimate portfolio age from first holding (use 3yr default if unknown)
    portfolio_years = 3.0

    cagr = compute_cagr(
        initial_value=total_invested,
        final_value=total_current,
        years=portfolio_years,
    )

    # Simulated XIRR using blended invested / current
    try:
        cashflows = simulate_sip_cashflows(
            total_invested=total_invested,
            months=int(portfolio_years * 12),
            final_value=total_current,
        )
    ...
    # Sharpe: use CAGR as proxy return, std dev estimated ~15% for equity blend
    ...
    with pm2:
        if xirr_val is not None:
            ...
            st.markdown(f'<div class="metric-strip"><div class="label">XIRR (simulated SIP)</div><div class="value {cls}">{xirr_val*100:.2f}%</div></div>', unsafe_allow_html=True)
        else:
    ...
    with pm4:
        ...
        cls = "positive" if gain_abs >= 0 else "negative"
        st.markdown(f'<div class="metric-strip"><div class="label">Absolute Gain</div><div class="value {cls}">₹{gain_abs/1e5:.2f}L</div></div>', unsafe_allow_html=True)

    st.caption("ℹ️ XIRR uses simulated equal-monthly SIP cashflows. CAGR assumes portfolio age of 3 years. Sharpe uses estimated 15% volatility for equity blend.")
...
weights_map  = {}
total_inv = sum(h["invested_amount"] for h in holdings) or 1
for h in holdings:
    if h.get("scheme_code"):
        weights_map[h["scheme_code"]] = h["invested_amount"] / total_inv

if scheme_codes:
    try:
        sector_alloc = compute_portfolio_sector_allocation(
        ...
        if sector_alloc:
            ...
            with sa2:
                fig_pie = go.Figure(go.Pie(
                    labels=sec_labels, values=sec_vals,
                    hole=0.45,
            ...
            st.info("No sector data available for current holdings. Run holdings_fetcher to populate.")
...
if len(scheme_codes) >= 2:
    try:
        ...
        if overlap_matrix is not None:
            fund_names = [
                next((h["scheme_name"] for h in holdings if h["scheme_code"] == sc), str(sc))
                for sc in scheme_codes
            ]
            # Shorten names for display
            short_names = [n.split("Fund")[0].strip()[:25] + "…" if len(n) > 28 else n for n in fund_names]

            import numpy as np
            matrix_vals = [[overlap_matrix.get((sc1, sc2), 0.0) * 100
                            for sc2 in scheme_codes] for sc1 in scheme_codes]
            ...
            fig_heat = go.Figure(go.Heatmap(
                ...
                zmin=0, zmax=100,
                text=[[f"{v:.1f}%" for v in row] for row in matrix_vals],
                texttemplate="%{text}",
                textfont={"size": 11},
                showscale=True,
            ...
            fig_heat.update_layout(
                title="Pairwise Portfolio Overlap (%)",
    ...
    st.info("Need at least 2 holdings with matching scheme codes to compute overlap.")
