...
def build_context(client, goals, holdings):
    ...
    gain   = cur - inv
    return {
        "client": {"name":client.get("name"),"age":client.get("age"),"annual_income":client.get("annual_income"),
                   "dependants":client.get("dependants"),"risk_profile":client.get("risk_profile"),"risk_score":client.get("risk_score")},
        "goals": [{"name":g.goal_name,"type":g.goal_type,"target":g.target_amount,"target_year":g.target_year,"monthly_sip":g.monthly_sip} for g in goals],
        "portfolio": {"num_holdings":len(holdings),"total_invested":inv,"total_current":cur,"absolute_gain":gain,"gain_pct":(gain/inv*100) if inv>0 else 0},
...
if "last_report" in st.session_state:
    ...
    download_ph.download_button(label="⬇️  Download as Markdown",
