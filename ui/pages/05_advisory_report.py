...
<style>
  ...
  .report-container h2 { font-family: 'DM Serif Display', serif !important; font-size: 1.2rem; color: #58A6FF !important; }
...
with st.sidebar:
    ...
    st.caption("Stream a full AI-generated financial plan.")
...
def build_report_context(client, goals, holdings):
    ...
    return {
        "client": {"name": client.get("name"), "age": client.get("age"),
                   "annual_income": client.get("annual_income"), "dependants": client.get("dependants"),
                   "risk_profile": client.get("risk_profile"), "risk_score": client.get("risk_score")},
        "goals": [{"name": g.goal_name, "type": g.goal_type, "target": g.target_amount,
                   "target_year": g.target_year, "monthly_sip": g.monthly_sip} for g in goals],
        "portfolio": {"num_holdings": len(holdings), "total_invested": total_invested,
                      "total_current": total_current, "absolute_gain": gain, "gain_pct": gain_pct},
        "performance": {"cagr": cagr, "xirr": xirr, "sharpe": sharpe},
        "holdings": [{"name": h.get("scheme_name"), "invested": h.get("invested_amount"), "current": h.get("current_value")} for h in holdings],
...
with st.expander("⚙️ Customise report focus (optional)"):
    extra_instructions = st.text_area("Additional instructions for Claude",
        placeholder="e.g. Focus on tax-saving strategies. Include NPS recommendations.", height=80)
    include_sections = st.multiselect("Sections to include",
        ["Executive Summary","Risk Profile Analysis","Goal Review","Portfolio Health","Fund Recommendations","Tax Planning","Action Items"],
        default=["Executive Summary","Risk Profile Analysis","Goal Review","Portfolio Health","Fund Recommendations","Action Items"])
...
if generate_btn:
    ...
    full_report = ""
    report_placeholder.markdown('<div class="report-container"><em style="color:#8B949E;">Generating report…</em></div>', unsafe_allow_html=True)
    try:
        for chunk in stream_advisory_narrative(client_context=context):
            full_report += chunk
            report_placeholder.markdown(f'<div class="report-container">{full_report}</div>', unsafe_allow_html=True)
    ...
    report_placeholder.markdown(f'<div class="report-container">{st.session_state["last_report"]}</div>', unsafe_allow_html=True)
...
if "last_report" in st.session_state:
    ...
    download_placeholder.download_button(
        ...
        file_name=f"advisory_report_{name_safe}_{date.today().isoformat()}.md",
