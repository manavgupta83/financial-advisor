...
Flow:
  1. KYC form — name, email, phone, age, income, dependants, PAN
  2. Risk questionnaire — 10 SEBI-aligned questions rendered as radio buttons
  3. Submit → computes risk profile, persists to DB, shows result card
...
# ── Shared CSS (same token system as app.py) ──────────────────────────────────
st.markdown("""
<style>
  ...
  .form-section {
    ...
    border-radius: 10px; padding: 1.5rem 1.75rem; margin-bottom: 1.25rem;
  }
  .form-section-title {
    font-size: 0.7rem; font-weight: 700; letter-spacing: 0.1em;
    text-transform: uppercase; color: #3FB950; margin-bottom: 1rem;
  }
...
# ── Sidebar client selector mirror ───────────────────────────────────────────
with st.sidebar:
...
with tab_new:
    with st.form("onboarding_form"):
        ...
        st.caption("Select the option that best describes your situation. Responses are used to compute your SEBI risk category.")
    ...
    if submitted:
        ...
        # Clean up existing demo client with same email
        existing = get_client_by_email(email)
        if existing:
            st.warning(f"A client with email **{email}** already exists (ID {existing.id}). Updating their risk profile.")
            ...
            client = create_client(
                ...
                annual_income=annual_income, dependants=dependants, pan=pan,
            )
            profile = compute_risk_profile(
                responses=responses,
                age=age,
        ...
        <div class="profile-card" style="border-top-color:{accent};">
          ...
          <p style="color:#8B949E;margin:0.4rem 0 1rem;font-size:0.85rem;">
            Raw score: {profile.raw_score} · Adjusted score: {profile.adjusted_score}
          </p>
...
with tab_view:
    ...
    if not os.path.exists(DATABASE_PATH):
        st.info("Database not yet initialised. Create a client first.")
        st.stop()

    conn = sqlite3.connect(DATABASE_PATH)
    conn.row_factory = sqlite3.Row
    rows = conn.execute(
        "SELECT id, name, email, age, annual_income, risk_profile, risk_score FROM clients ORDER BY name"
    ).fetchall()
    conn.close()
    ...
    if not rows:
        ...
        for row in rows:
            ...
            color_map = {
                "Conservative":"#58A6FF","Moderate":"#3FB950",
                "Aggressive":"#E3B341","Very Aggressive":"#F85149",
            }
            ...
            with st.expander(f"**{row['name']}**  ·  {row['email']}"):
                ...
                cc4.metric("Risk Score", score)
                if st.button(f"Select as Active Client", key=f"sel_{row['id']}"):
